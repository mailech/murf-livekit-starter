...
class Assistant(Agent):
    def __init__(self, profile: LocaleProfile, room: rtc.Room | None = None) -> None:
        ...
        self._active_key = "nova"  # who currently holds the conversation
    ...
    # --- Day 9: handing off to a specialist -------------------------------
    #
    # Three separate tools rather than one with a "which" argument. The model
    # routes on the tool DESCRIPTION, so three specific descriptions route far
    # more accurately than one generic tool with an enum.

    async def _hand_off(self, key: str, context: RunContext, reason: str = "") -> None:
        """Swap in a specialist, carrying the conversation with it.

        Use context.session, NEVER self.session. These tools are also handed to
        the specialists so they can route sideways, and while a specialist is
        active this Assistant is not — its own .session reference is stale, so
        self.session.update_agent() silently fails to switch anyone.
        """
        session = context.session
        # Carry the live conversation, whoever is currently holding it.
        current = session.current_agent
        chat_ctx = current.chat_ctx if current else self.chat_ctx

        specialist = SPECIALISTS[key](main=self, chat_ctx=chat_ctx)
        specialist.handoff_reason = reason
        specialist.handoff_from = self._active_key
        self._active_key = key

        logger.info("handoff", extra={"to": key, "why": reason})
        session.update_agent(specialist)

    @function_tool
    async def transfer_to_algo(self, context: RunContext, reason: str = "") -> None:
        """Hand over to అల్గో, the data structures and algorithms specialist.

        Use when the student wants to LEARN or go deep on an algorithm or data
        structure: sorting, searching, trees, graphs, dynamic programming,
        recursion as a technique, or time and space complexity.

        Say one short sentence first — "అల్గో కి ఇస్తున్నా, వాడు DSA లో మస్తు" —
        then call this. Algo introduces itself.

        Do NOT use this for fixing a student's broken program (that is Keerthi)
        or for interview practice (that is Vikram).

        Args:
            reason: One short phrase, in the student's language, on why you are
                handing over. Shown on their screen — "DSA లోతుగా చెప్పడానికి".
        """
        await self._hand_off("algo", context, reason)

    @function_tool
    async def transfer_to_debug(self, context: RunContext, reason: str = "") -> None:
        """Hand over to కీర్తి, the debugging specialist.

        Use when the student has a SPECIFIC program that is not working: an
        error message, a crash, wrong output, code that hangs, something that
        "worked yesterday". Anything where the goal is to fix their code rather
        than learn a concept.

        Say one short sentence first, then call this.

        Do NOT use this for teaching an algorithm from scratch (that is Algo).

        Args:
            reason: One short phrase, in the student's language, on why you are
                handing over. Shown on their screen — "code error fix చేయడానికి".
        """
        await self._hand_off("keerthi", context, reason)

    @function_tool
    async def transfer_to_interview(
        self, context: RunContext, reason: str = ""
    ) -> None:
        """Hand over to विक्रम, the interview preparation specialist.

        Use when the student mentions interviews, placements, campus drives,
        mock questions, HR rounds, or wants to practise answering out loud.

        Warn them he speaks HINDI, not Telugu, and say why in half a sentence —
        placement interviews here are conducted in Hindi and English. Then call
        this.

        Do NOT use this for ordinary teaching or debugging.

        Args:
            reason: One short phrase on why you are handing over. Shown on
                their screen — "placement interview practice కోసం".
        """
        await self._hand_off("vikram", context, reason)
