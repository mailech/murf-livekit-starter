...
# Murf DOES speak Telugu — not as a te-IN voice id, but through multilingual
# voices whose supportedLocales map includes te-IN. The voice id and the locale
# are separate knobs: en-IN-samar + locale="te-IN" is native Telugu, not an
# English voice reading Telugu.
#
# Check with: GET /v1/speech/voices?model=FALCON  ->  each voice's
# "supportedLocales" object. Reading only the top-level "locale" field will
# make you conclude Telugu is unsupported. It is not.
#
# Falcon voices that support te-IN:
#   en-IN-samar (M), mr-IN-prathamesh (M), gu-IN-hardik (M),
#   en-IN-anisha (F), en-IN-pooja (F), en-IN-anusha (F), mr-IN-prajakta (F)
VOICE_TELUGU = "en-IN-samar"
VOICE_TELUGU_ALT_MALE = "mr-IN-prathamesh"
VOICE_TELUGU_FEMALE = "en-IN-anisha"
...
def resolve(
    ...
    if key in {"telangana", "andhra pradesh"}:
        return LocaleProfile(
            ...
            # Native Telugu: the voice is multilingual and the locale selects
            # which of its languages to speak. No transliteration involved.
            murf_voice_id=VOICE_TELUGU,
            murf_locale="te-IN",
            murf_style="Conversational",
            stt_language="te-IN",
            script="native",
            tts_transliterate=None,
        )
...
DEFAULT_PROFILE = resolve(name="", state="Telangana", district="Warangal")
